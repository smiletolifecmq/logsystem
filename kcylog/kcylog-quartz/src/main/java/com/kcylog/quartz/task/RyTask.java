package com.kcylog.quartz.task;
...
public class RyTask
{
    ...
    @Autowired
    private IFqTriggerService fqTriggerService;

    @Transactional
    public void syncBcProject() throws ParseException {
        System.out.println("同步百川项目");
        FqTrigger param = new FqTrigger();
        List<FqTrigger> list = fqTriggerService.selectFqTriggerList(param);
        for (FqTrigger mqMessage : list){
            //获取视图数据
