  ...
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="提前工期" align="center">
        <template slot-scope="scope">
          <el-tag type="danger" v-show="scope.row.oneCheckDays < 0">{{
            scope.row.oneCheckDays
          }}</el-tag>
          <el-tag type="success" v-show="scope.row.oneCheckDays >= 0">{{
            scope.row.oneCheckDays
          }}</el-tag>
        </template></el-table-column
      >
    ...
    >
      <el-form ref="formReviewCq" :model="formReviewCq" label-width="80px">
        <el-form-item label="超期备注" prop="oneCheckNotes">
          <el-input
            type="textarea"
            :rows="2"
            placeholder="请输入内容"
            v-model="formReviewCq.oneCheckNotes"
          >
          </el-input>
        </el-form-item>
      </el-form>
...
export default {
  ...
  methods: {
    ...
    submitFormCq() {
      if (!this.formReviewCq.oneCheckNotes) {
    ...
    getStatisticsData() {
      listProjectWaitOneCheck(
        ...
        if (myMap.has("工程测绘部")) {
          numData.gcchbNumWork = myMap.get("工程测绘部").oneCheckDays;
          numData.gcwbzs = myMap.get("工程测绘部").wbznum;
        }
        if (myMap.has("不动产测绘部")) {
          numData.bdcchbWork = myMap.get("不动产测绘部").oneCheckDays;
          numData.bdwbzs = myMap.get("不动产测绘部").wbznum;
        }
        if (myMap.has("管线工程部")) {
          numData.gxgcbWork = myMap.get("管线工程部").oneCheckDays;
          numData.gxwbzs = myMap.get("管线工程部").wbznum;
        }
        if (myMap.has("地理信息部")) {
          numData.dlxxbWork = myMap.get("地理信息部").oneCheckDays;
