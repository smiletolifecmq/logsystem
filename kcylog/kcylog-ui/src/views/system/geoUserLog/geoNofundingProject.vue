...
import {
  ...
  addProject,
  updateProject,
} from "@/api/system/geoNofundingProject";
...
export default {
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$modal
        .confirm('是否确认删除无经费服务项目统计编号为"' + ids + '"的数据项？')
        .then(function () {
    ...
    handleExport() {
      this.download(
        "system/geoNofundingProject/export",
        {
          ...this.queryParams,
        },
        `无经费服务类型项目统计表_${new Date().getTime()}.xlsx`
      );
    },
  },
};
