...
import {
  ...
  addInfo,
  updateInfo,
} from "@/api/system/carReviewConfigInfo";
...
export default {
  ...
  created() {
    this.reviewConfigId =
      this.$route.params && this.$route.params.reviewConfigId;
  ...
  methods: {
    ...
    selectReviewer(item) {
      ...
      this.form.reviewConfigId = this.reviewConfigId;
    },
    ...
    getList() {
      this.loading = true;
      this.queryParams.reviewConfigId = this.reviewConfigId;
    ...
    handleDelete(row) {
      ...
      this.$modal
        .confirm('是否确认删除流程配置过程编号为"' + infoIds + '"的数据项？')
        .then(function () {
